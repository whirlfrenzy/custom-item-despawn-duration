...
public abstract class MidnightConfig {
    ...
    public static void init(String modid, Class<? extends MidnightConfig> config) {
        ...
        for (Field field : config.getFields()) {
            ...
            if ((field.isAnnotationPresent(Entry.class) || field.isAnnotationPresent(Comment.class)) && !field.isAnnotationPresent(Server.class) && !field.isAnnotationPresent(Hidden.class) && PlatformSpecificHelper.isClientSide())
    ...
    private static void textField(EntryInfo info, Function<String,Number> f, Pattern pattern, double min, double max, boolean cast) {
        ...
        info.widget = (BiFunction<TextFieldWidget, ButtonWidget, Predicate<String>>) (t, b) -> s -> {
            ...
            t.setUneditableColor(inLimits ? 0x404040 : 0x840000);
            ...
            if (this.list != null) {
                for (ButtonEntry entry : this.list.children()) {
                    if (entry.buttons != null && entry.buttons.size() > 1 && entry.buttons.get(1) instanceof ButtonWidget button) {
                        button.active = entry.info.error == null && !entry.info.disabled && !Objects.equals(entry.info.value.toString(), entry.info.defaultValue.toString());
                    }
                }
            }
        }
    ...
    public static class MidnightConfigListWidget extends ElementListWidget<ButtonEntry> {
        ...
        public void addButton(List<ClickableWidget> buttons, Text text, EntryInfo info) {
            ...
            this.addEntry(info.buttonEntry);
        }
        ...
        public void render(DrawContext context, int index, int y, int x, int entryWidth, int entryHeight, int mouseX, int mouseY, boolean hovered, float tickDelta) {
            ...
            if (text != null && (!text.getString().contains("spacer") || !buttons.isEmpty())) {
                ...
                for(Iterator<OrderedText> textIterator = textRenderer.wrapLines(text, (buttons.size() > 1 ? buttons.get(1).getX()-24 : MinecraftClient.getInstance().getWindow().getScaledWidth() - 24)).iterator(); textIterator.hasNext(); wrappedY += 9) {
                    context.drawTextWithShadow(textRenderer, textIterator.next(), (centered) ? (MinecraftClient.getInstance().getWindow().getScaledWidth() / 2 - (textRenderer.getWidth(text) / 2)) : 12, wrappedY + 5, this.info.disabled ? 0x444444 : 0xFFFFFF);
                }
            }
        }
